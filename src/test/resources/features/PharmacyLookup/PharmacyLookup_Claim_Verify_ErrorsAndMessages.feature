Feature: PharmacyLookup search verify Errors & Messages
  ...
  Scenario Outline: Pharmacy Lookup search with valid data
    ...
    Examples: 
      ...
      | Miscellaneous->RxNova Connect |  1710927462 | 120005552999 | Member ID^90413000100 | Date of Birth^01/01/1940 | Member Name^MBRNONBRIDGECUST RWT | Error^ | Date^01/16/2020 | Time^10:27:28 | CICS Status^PWE | Cust ID^319 - 0319HUMANA | Client ID^9007 - NBRG--CLNT1 - NONBRG CLIENT--PDP | Group^2 - RWNBRG REGR2 DONT USE REWORK10 |
  ...
  @Functional
  Scenario Outline: Verify Errors and Messages
    ...
    Then User verifies the Errors and Messages    
      ...
      | Argus          | <Argus>          |
      | Action         | <Action>  	      |
      | ArgusErrorText | <ArgusErrorText> | 
      | Qualifier      | <Qualifier>      |
      | Continue       | <Continue>       |
      | Message        | <Message>        |      
       
    Examples: 
      | NCPDP  | RejectField   | Argus     | Action             | ArgusErrorText                                                                     | Qualifier  | Continue  | Message  |  
      | NCPDP^ | Reject Field^ | Argus^020 | Action^Information | Argus Error Text^INGREDIENT COST GREATER THAN NDC AMOUNT BY MORE THAN PLAN EXPECTS | Qualifier^ | Continue^ | Message^ |  
